...
public class CLJRegistrationController
{

	private static final Logger LOGGER = LoggerFactory.getLogger(CLJRegistrationController.class);
	...
	private static final String REGISTRATION_MODEL_FIELD_FIRSTNAME = "firstName";
	...
	private static final String DOMAIN_EMAIL = "admin@" + DOMAIN;

	private static final String DOMAIN_DESCRIPTION = "Caterham Ladies Joggers";

	private static final Map<String, String> CONFIRMATION_MODEL_OBJECTS = Map.of("domain", DOMAIN,
	                                                                             "domainEmail", DOMAIN_EMAIL,
	                                                                             "domainDescription", DOMAIN_DESCRIPTION,
	                                                                             "facebookUrl", "https://www.facebook.com/caterhamladiesjoggers");

	private static final Map<String, String> NOTIFICATION_MODEL_OBJECTS = Map.of("notificationCategory", "New Registration");
	...
	@Post(value = "/register/{emailAddress}", consumes = {"application/x-www-form-urlencoded", "application/json"})
	...
	private void processRegistration(SingleEmitter<HttpResponse> emitter, String recipientAddress, UserDetail user, boolean isSmokeTest) throws MailServiceException
	{
		...
		userModel.put(REGISTRATION_MODEL_FIELD_FIRSTNAME, user.getFirstName());
	...
	{
		body.putAll(NOTIFICATION_MODEL_OBJECTS);
		...
		sendEMail(DOMAIN_EMAIL, "New Registration", templateOutput.toString(), isSmokeTest);
	}
	...
	{
		body.putAll(CONFIRMATION_MODEL_OBJECTS);
	...
	{
		mailService.sendHtmlMail(DOMAIN_EMAIL + "(" + DOMAIN_DESCRIPTION + ")", recipientAddress, subject, mailContent, isSmokeTest);
	}
}
